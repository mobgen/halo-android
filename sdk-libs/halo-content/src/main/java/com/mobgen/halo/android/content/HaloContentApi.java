...
public class HaloContentApi extends HaloPluginApi {
    ...
    @NonNull
    @Api(2.3)
    public void createAutoGeneratedTables(GeneratedHaloDatabase migration) {
        //perfom this operation on interactor
        migration.updateDatabase(halo().framework().storage(HaloContentContract.HALO_CONTENT_STORAGE).db().getDatabase());
    }
