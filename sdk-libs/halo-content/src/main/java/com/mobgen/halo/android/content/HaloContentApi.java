...
public class HaloContentApi extends HaloPluginApi {
   ...
   // @HaloQuery(name="selectAll",query="select * from MY_TABLE where id = ?id:Integer orderBy id DESCENDANT")
    ...
    @NonNull
    @Api(2.2)
    public static HaloContentApi createAutoGeneratedTables(@NonNull Halo halo, String locale , HaloDatabaseMigration migration) {
        HaloStorageApi storageApi = halo.framework().createStorage(StorageConfig.builder()
                .storageName("com.mobgen.halo.android.content.halo-content-generated")
                .databaseVersion(migration.getDatabaseVersion())
                .errorHandler(new HaloDatabaseErrorHandler())
                .addMigrations(migration)
                .build()
        );
       return new HaloContentApi(halo,
                createContentSearchRepository(halo, storageApi),
                createContentSyncRepository(halo, storageApi),
                locale);
    }
