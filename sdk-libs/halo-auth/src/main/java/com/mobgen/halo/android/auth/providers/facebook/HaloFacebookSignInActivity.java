...
public class HaloFacebookSignInActivity extends FragmentActivity implements FacebookCallback<LoginResult>, GraphRequest.GraphJSONObjectCallback {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (HaloUtils.isNetworkConnected(this)) {
            ...
            onError(new HaloConnectionException("Network is not available", null), HaloAuthApi.Error.CODE_NO_INTERNET);
        }
    }
    ...
    public void onError(@NonNull FacebookException e) {
        if (isPermissionRevoked(e)) {
            ...
            onError(e, HaloAuthApi.Error.CODE_NO_INTERNET);
        } else {
            onError(e, HaloAuthApi.Error.CODE_PROVIDER_ERROR);
        }
    }
    ...
    public void onCompleted(@Nullable JSONObject object, @NonNull GraphResponse response) {
        if (response.getError() != null) {
            ...
            onError(new IllegalStateException("Something wrong happened with the facebook request. No data available"), HaloAuthApi.Error.CODE_NO_INTERNET);
        }
    }
