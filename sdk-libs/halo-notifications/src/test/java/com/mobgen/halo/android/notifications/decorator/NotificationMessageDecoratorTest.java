...
public class NotificationMessageDecoratorTest extends BaseNotificationDecoratorTest<NotificationMessageDecorator> {

    @Test
    public void messageTest() {
        final boolean[] marker = new boolean[1];
        final String text = "This is my Text";
        Bundle bundle = BundleTestUtils.builder().putString("body", text).build();
        when(getBuilder().setContentText(any(String.class))).then(new Answer<Object>() {
            @Override
    ...
    @Override
    public NotificationMessageDecorator createDecorator() {
        return new NotificationMessageDecorator(null);
    }
}
