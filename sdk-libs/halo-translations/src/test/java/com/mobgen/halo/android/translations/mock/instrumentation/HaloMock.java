package com.mobgen.halo.android.translations.mock.instrumentation;
...
public class HaloMock {
    ...
    @NonNull
    public static HaloConfig.Builder givenADelayedExecutorWithParserConfig(String urlEndpoint) {
        final Handler handler = new Handler();
        return HaloConfig.builder(RuntimeEnvironment.application)
                .addEndpoint(new HaloEndpoint(HaloNetworkConstants.HALO_ENDPOINT_ID, urlEndpoint))
                .threadManager(new HaloThreadManager() {
                    @Override
                    public Future<?> enqueue(int thread, @NonNull final Runnable runnable) {
